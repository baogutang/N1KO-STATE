...
struct SessionClientInfo: Codable, Equatable, Sendable {
    ...
    nonisolated var isQoderCLIClient: Bool {
        let rawProfileID = profileID?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if rawProfileID == "qoder-cli" || rawProfileID == "qoder-cn-cli" { return true }
        let rawName = name?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if ["qoder cli", "qoder-cn-cli", "qoder cn cli", "qoderclicn"].contains(rawName) {
            return true
        }
    ...
    nonisolated var isQoderNotifyOnlyIDEClient: Bool {
        let normalized = normalizedForClaudeRouting()
        let profileID = normalized.profileID?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        ...
        return [
            ...
            case "com.qoder.ide", "com.aliyun.lingma.ide": return true
