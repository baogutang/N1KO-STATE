...
struct SessionQuestionForm: View {
    ...
    var body: some View {
        ...
        .onChange(of: otherAnswers) { _ in
            notifyInteractionState()
            persistDraft()
        }
        .onChange(of: focusedIslandTextQuestionID) { _ in notifyInteractionState() }
        .onChange(of: focusedSecureQuestionID) { _ in notifyInteractionState() }
        .onDisappear {
