...
final class DetachedIslandWindowController: NSWindowController, NSWindowDelegate {
    ...
    init(
        ...
        // N1KO intentionally diverges from Ping-Island here: the detached pet
        // remains an ordinary-Space surface and can never flash into a native
        // fullscreen Space before detection catches up.
        window.collectionBehavior = [.moveToActiveSpace, .stationary, .ignoresCycle, .fullScreenNone]
    ...
    private func bindWindowSizeUpdates() {
        ...
        AppSettings.shared.$agentNotificationMuteUntil
            .dropFirst()
            .receive(on: DispatchQueue.main)
    ...
    static func windowLayout(
        ...
        let additionalFooterHeight: CGFloat = {
            guard AppSettings.shared.showUsage,
            ...
            let providers = UsageSummaryPresenter.providers(
                ...
                mode: AppSettings.shared.usageValueMode,
            ...
            return UsageSummaryPresenter.shouldShowSummary(
                for: route,
                showUsage: AppSettings.shared.showUsage,
                providers: providers
    ...
    private func presentFloatingSettingsHintIfNeeded() {
        guard AppSettings.shared.floatingPetSettingsHintPending else { return }

        AppSettings.shared.floatingPetSettingsHintPending = false
        floatingSettingsHintDismissWorkItem?.cancel()
    ...
    private func handleCompletionNotificationChange(_ instances: [SessionState]) {
        ...
        if AppSettings.shared.agentNotificationsTemporarilyMuted {
    ...
    private func shouldQueueCompletedNotification(
        ...
        SessionCompletionNotificationPolicy.shouldQueueCompletedNotification(
            ...
            isEnabled: AppSettings.shared.autoOpenCompletionPanel
        )
    }
    ...
    private func shouldQueueCompactedNotification(
        ...
        SessionCompletionNotificationPolicy.shouldQueueCompactedNotification(
            for: session,
            previousPhase: previousPhase,
            isEnabled: AppSettings.shared.autoOpenCompactedNotificationPanel
        )
    }
    ...
    private func maybePresentNextCompletionNotification() {
        guard !AppSettings.shared.agentNotificationsTemporarilyMuted else { return }
