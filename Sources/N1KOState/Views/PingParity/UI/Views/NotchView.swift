...
struct NotchView: View {
    ...
    @State private var lastObservedStatus: NotchStatus = .closed
    ...
    private var lifecycleBody: some View {
        presentedBody
            ...
            .onChange(of: viewModel.status) { newStatus in
                let oldStatus = lastObservedStatus
                lastObservedStatus = newStatus
    ...
    private var settingsAwareBody: some View {
        lifecycleBody
            .onChange(of: settings.autoOpenCompletionPanel) { isEnabled in
                if !isEnabled {
            ...
            .onChange(of: settings.autoOpenCompactedNotificationPanel) { isEnabled in
                if !isEnabled {
                    removeCompletionNotifications(
            ...
            .onChange(of: settings.temporarilyMuteNotificationsUntil) { mutedUntil in
            ...
            .onChange(of: settings.autoHideWhenIdle) { _ in
                handleProcessingChange()
            }
    }
    ...
    private var contentTypeAwareBody: some View {
        settingsAwareBody
            .onChange(of: viewModel.contentType.id) { _ in
                maybePresentNextCompletionNotification()
            }
    ...
    private var visibilityAwareBody: some View {
        contentTypeAwareBody
            .onChange(of: viewModel.isFullscreenEdgeRevealActive) { isActive in
            ...
            .onChange(of: viewModel.isFullscreenBrowserHiddenActive) { isActive in
                if isActive {
                    isVisible = false
                } else {
                    handleProcessingChange()
            ...
            .onChange(of: viewModel.isIdleAutoHiddenActive) { isHidden in
            ...
            .onChange(of: viewModel.isQuietBackgroundPresentationActive) { isActive in
                handleProcessingChange()
                if !isActive {
                    scheduleDetachmentHintPresentationIfNeeded(delay: Self.detachmentHintRetryDelay)
                }
            ...
            .onChange(of: viewModel.isFullscreenPhysicalNotchCompactActive) { isActive in
                if !isActive {
                    scheduleDetachmentHintPresentationIfNeeded(delay: Self.detachmentHintRetryDelay)
                }
            }
            .onChange(of: settings.surfaceMode) { _ in
                scheduleDetachmentHintPresentationIfNeeded(delay: Self.detachmentHintRetryDelay)
            }
            .onChange(of: settings.notchDetachmentHintPending) { isPending in
                if isPending {
                    scheduleDetachmentHintPresentationIfNeeded(delay: Self.detachmentHintRetryDelay)
                } else {
                    cancelScheduledDetachmentHintPresentation()
    ...
    private var shortcutAwareBody: some View {
        visibilityAwareBody
            .onReceive(NotificationCenter.default.publisher(for: .n1koOpenActiveSessionShortcut)) { _ in
                handleOpenActiveSessionShortcut()
            }
            .onReceive(NotificationCenter.default.publisher(for: .n1koOpenSessionListShortcut)) { _ in
                handleOpenSessionListShortcut()
            }
            .onReceive(NotificationCenter.default.publisher(for: .n1koPresentNotchDetachmentHint)) { _ in
                presentDetachmentHintIfNeeded(force: true)
            }
            .onReceive(NotificationCenter.default.publisher(for: .n1koHookWalkthroughDemoShouldCloseNotch)) { _ in
    ...
    private func maybePresentNextCompletionNotification() {
        ...
        markCompletionNotificationConsumed(nextNotification)
    ...
    private func dequeueNextPresentableCompletionNotification() -> SessionCompletionNotification? {
        while !completionNotificationQueue.isEmpty {
            ...
            guard isCompletionNotificationPresentable(notification) else {
                markCompletionNotificationConsumed(notification)
                continue
            }
            return notification
        }
    ...
    private func removeCompletionNotifications(
        ...
        let removedQueuedNotifications = completionNotificationQueue.filter { shouldRemove($0.kind) }
        completionNotificationQueue.removeAll { shouldRemove($0.kind) }
        for notification in removedQueuedNotifications {
            markCompletionNotificationConsumed(notification)
        }
    ...
    private func isCompletionNotificationConsumed(_ notification: SessionCompletionNotification) -> Bool {
        isCompletionNotificationConsumed(session: notification.session, kind: notification.kind)
    }

    private func markCompletionNotificationConsumed(
        session: SessionState,
        kind: SessionCompletionNotification.Kind
    ) {
