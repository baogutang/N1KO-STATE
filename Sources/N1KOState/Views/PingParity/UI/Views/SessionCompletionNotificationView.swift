...
private struct SessionCompletionHitShape: Shape {
    let isBubble: Bool

    func path(in rect: CGRect) -> Path {
        if isBubble { return Rectangle().path(in: rect) }
...
struct SessionCompletionNotificationView: View {
    ...
    var body: some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .contentShape(SessionCompletionHitShape(isBubble: presentationStyle == .bubble))
